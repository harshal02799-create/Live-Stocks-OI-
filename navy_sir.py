...
def scan_all(symbols, lot_dict):
    ...
    bullish_df = pd.DataFrame(bullish_rows, columns=columns)
    bearish_df = pd.DataFrame(bearish_rows, columns=columns)
...
for symbol in list(st.session_state.breakdown_tracker.keys()):
    ...
    if "Symbol" in bearish_df.columns and symbol not in bearish_df["Symbol"].values:
